...
(function () {
  ...
  const bridgeLog = typeof createLogger === 'function'
  ...
  window.addEventListener('message', (event) => {
    if (event.source !== window || event.data?.type !== MSG || !event.data.token) return;
    const preview = event.data.token.length > 20
