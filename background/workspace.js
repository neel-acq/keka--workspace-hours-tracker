...
async function getWorkspaceTab() {
  const tabs = await chrome.tabs.query({
    url: "https://workspace.acquaintsoft.com/*",
  });
...
async function getWorkspaceAuth() {
  ...
  const workspaceCookies = cookies.filter((c) =>
    c.domain?.includes("acquaintsoft.com"),
  );
  const cookieHeader = workspaceCookies
...
async function postTimerTrackingViaTab(
  taskId,
  timerId = "",
  note = "",
  csrfToken = null,
    ...
    func: async (postBody) => {
      try {
        const res = await fetch(
          "/admin/tasks/timer_tracking?single_task=true",
          {
            method: "POST",
            headers: {
              "Content-Type":
...
async function startWorkspaceTimer(taskId, note = "") {
  ...
  if (!localAuth.success) {
    ...
    if (tab && csrf) {
      ...
      return {
        success: false,
        error: localAuth.error || "Workspace session not found",
      };
    }
  }
...
async function fetchTimesheetsToday(auth) {
  if (auth.tabOnly) {
    ...
    if (!data)
      throw new Error("Timesheet fetch failed — keep Workspace tab open");
    const summary = parseTimesheetSummary(data);
    wsLog(
      "timesheet parsed (tab):",
      summary.totalHours,
      "entries:",
...
async function fetchWorkspaceData() {
  ...
  if (
    ...
    if (apiResult.reason !== "no_keka_token") {
      wsWarn(
        "API workspace fetch failed, falling back local:",
        apiResult.error,
      );
    }
...
function handleWorkspaceMessage(message, sendResponse) {
  ...
  if (message.type === "UPDATE_WORKSPACE_ALARM_INTERVAL") {
    ...
    chrome.alarms.clear("workspace_timer_monitor", () => {
      chrome.alarms.create("workspace_timer_monitor", {
...
function hasKekaInToday(scrapedAttendance) {
  ...
  if (todayEntry.inOutArray?.length) {
    return todayEntry.inOutArray.some(
      (s) => s.type === "IN" && s.time && s.time !== "MISSING",
    );
  }
  ...
  if (
    ...
    await dispatchTrackerAlert({
      ...
      actions: [
        {
          id: "open_workspace",
          label: alertT("alert_open_workspace", lang),
          primary: true,
        },
