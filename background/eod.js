...
function initEodModule() {
  ...
  chrome.webRequest.onBeforeRequest.addListener(
    (details) => {
      if (!details.url.includes("timer_tracking") || details.method !== "POST")
        return;
  ...
  chrome.webRequest.onCompleted.addListener(
    async (details) => {
      ...
      const { eodEnabled } = await chrome.storage.local.get({
        eodEnabled: true,
      });
      ...
      if (!pending) {
        eodLog.warn(
          "timer_tracking completed without captured body — assuming stop",
        );
      }
...
async function refreshTeamsTokenIfNeeded() {
  ...
  const now = Date.now();
  const needsRefresh =
    !teamsTokenExpiry || teamsTokenExpiry <= now + TEAMS_REFRESH_MARGIN_MS;
  if (!needsRefresh) return;

  teamsCaptureLog("token expired or expiring soon — opening Teams for capture");

  const patterns = ["https://teams.live.com/*", "https://*.teams.live.com/*"];
  ...
  if (existingTabs.length) {
    ...
    if (normalWindows.length > 0) {
      const targetWindow =
        normalWindows.find((w) => w.focused) || normalWindows[0];
...
function saveTeamsToken(token, source = "unknown") {
  ...
  try {
    ...
    if (payload?.exp) {
      ...
      storageUpdate.teamsTokenExpiry = Math.max(
        expMs,
...
async function fetchTeamsUserIdFromStorage() {
  const { teamsSkypeToken } = await chrome.storage.local.get({
    teamsSkypeToken: "",
  });
  if (!teamsSkypeToken) {
...
function handleEodMessage(message, sendResponse) {
  ...
  if (message.type === "INJECT_TEAMS_TOKEN_HOOKS" && message.tabId) {
    injectTeamsTokenHooks(message.tabId).then(() =>
      sendResponse({ success: true }),
    );
    return true;
  }
  ...
  if (
    message.type === "SEND_TEAMS_MESSAGE" ||
    message.action === "sendTeamsMessage"
  ) {
...
async function handleSendTeamsMessage(messageText) {
  ...
  if (!teamsFromId) {
    return {
      success: false,
      error:
        "Teams user ID not found. Open Teams or use Fetch User ID in Workspace tab.",
    };
  }
  ...
  if (!config.teamsSkypeToken || Date.now() > config.teamsTokenExpiry) {
    return {
      success: false,
      error: "Teams token is missing or expired. Open Teams to refresh it.",
    };
  }
...
async function computeSmartEodSuggestion(reason) {
  ...
  const presets =
    Array.isArray(teamsPrewrittenMessages) && teamsPrewrittenMessages.length
