...
async function apiFetch(path, options = {}) {
    if (!API_ENABLED) {
        return { success: false, error: 'API not configured. Set API_BASE_URL in config.js' };
    }

    const token = await getKekaToken();
    if (!token) {
        return { success: false, error: 'No Keka token. Visit Keka to capture token.' };
    }
...
async function apiCheckAlerts(options) {
    ...
    if (options.notificationsEnabled === false) params.set('notificationsEnabled', 'false');
    if (options.workspaceAlertsEnabled === false) params.set('workspaceAlertsEnabled', 'false');
    if (options.workspaceAlertInterval) params.set('workspaceAlertInterval', String(options.workspaceAlertInterval));
...
async function collectWorkspaceCookiesForApi() {
    ...
    for (const url of urls) {
        ...
        list.forEach((c) => { cookies[c.name] = c.value; });
    }
...
async function syncWorkspaceCredentialsToApi() {
    ...
    if (!session) return;
    const result = await apiSyncWorkspaceSession(session.csrfToken, session.cookies);
    if (result.success) {
...
async function syncTeamsCredentialsToApi() {
    ...
    const result = await apiSyncTeamsCredentials({
        ...
        tokenExpiry: data.teamsTokenExpiry ? new Date(data.teamsTokenExpiry).toISOString() : null,
